...
func _set_name() -> void:
	if GameLoader.current_greenhouse == "": return
	_greenhouse_name = GameLoader.current_greenhouse

func _set_data() -> void:
	var _path = DirAccess.open("user://game/data/farm/greenhouses")
	var _path_file_read = FileAccess.open("user://game/data/farm/greenhouses/" + str(_greenhouse_name) + ".json", FileAccess.READ)
...
func _load_fertilizers(_fertilizer_data:Dictionary) -> void:
	if _fertilizer_data.is_empty(): return
...
func _load_plants(_plant_data:Dictionary) -> void:
	if _plant_data.is_empty(): return
