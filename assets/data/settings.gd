...
# - settings_load(values:Dictionary) - Установить значения переменных
...
func settings_load(values:Dictionary) -> void:
    language = int(values["language"])                    if values.has("language") else 1
    movement_type = int(values["control"]["movement"])    if values.has("movement") else 1
    vsync = values["graphic"]["vsync"]                    if values.has("graphic") && values["graphic"].has("vsync")      else false
    fullscreen = values["graphic"]["screen"]              if values.has("graphic") && values["graphic"].has("screen")     else true
    fps = int(values["graphic"]["fps"])                   if values.has("graphic") && values["graphic"].has("fps")        else 1
    general_volume  = int(values["volumes"]["general"])   if values.has("volumes") && values["volumes"].has("general")    else 100
...
func settings_volume_apply() -> void:
    AudioServer.set_bus_volume_db(AudioServer.get_bus_index('Master'), 
        (20.0 * log(clamp(general_volume / 100.0, 0.001, 1.0)) / log(10.0)))
    AudioServer.set_bus_volume_db(AudioServer.get_bus_index('Music'), 
        (20.0 * log(clamp(music_volume / 100.0, 0.001, 1.0)) / log(10.0)))
    AudioServer.set_bus_volume_db(AudioServer.get_bus_index('Nature'), 
        (20.0 * log(clamp(nature_volume / 100.0, 0.001, 1.0)) / log(10.0)))
    AudioServer.set_bus_volume_db(AudioServer.get_bus_index('Radio'), 
        (20.0 * log(clamp(radio_volume / 100.0, 0.001, 1.0)) / log(10.0)))
