...
#
# ОСНОВНОЙ ФУНКЦИОНАЛ:
# - ui_add(object) - Добавление пользовательского интерфейса в контейнер UI, при условии что ее ранее не было
...
{
    PAUSE   = preload("res://assets/nodes/ui/interactive/pause/pause.tscn"),
    OPTIONS = preload("res://assets/nodes/ui/options/settings.tscn"),
    CREDITS = preload("res://assets/nodes/menu/credits.tscn"),
...
func _button_hovered(disabled:bool = false) -> void:
    if !disabled:
