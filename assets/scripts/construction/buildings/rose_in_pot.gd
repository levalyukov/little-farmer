...
var object:Dictionary = {
	"shadow" = load("res://assets/resources/buildings/rose_in_pot/shadow.png"),
	"default" = load("res://assets/resources/buildings/rose_in_pot/object_0.png"),
}
	...
	if visible:
		if object.has("shadow"):
			if object["shadow"] is CompressedTexture2D:
				canvas.create_shadow("rose_in_pot_shadow", object["shadow"], tilemap.local_to_map(position))
			else:
