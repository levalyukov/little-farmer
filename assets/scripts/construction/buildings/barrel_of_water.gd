...
var object:Dictionary = {
	"shadow" = load("res://assets/resources/buildings/barrel_of_water/shadow.png"),
	"default" = load("res://assets/resources/buildings/barrel_of_water/object_0.png"),
}
	...
	if visible:
		if object.has("shadow"):
			if object["shadow"] is CompressedTexture2D:
				canvas.create_shadow("barrel_of_water_shadow", object["shadow"], tilemap.local_to_map(position))
			else:
