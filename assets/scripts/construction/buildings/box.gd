...
var object:Dictionary = {
	"shadow" = load("res://assets/resources/buildings/box/shadow.png"),
	"default" = load("res://assets/resources/buildings/box/object_0.png"),
}
	...
	if visible:
		if object.has("shadow"):
			if object["shadow"] is CompressedTexture2D:
				canvas.create_shadow("box_shadow", object["shadow"], tilemap.local_to_map(position))
			else:
