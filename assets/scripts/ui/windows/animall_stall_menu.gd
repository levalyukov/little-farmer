...
@onready
var mainContainer: VBoxContainer = $NinePatchRect/HBoxContainer/MarginContainer/NinePatchRect/ScrollContainer/MarginContainer/VBoxContainer
...
func _get_animals():
	...
	for i in animalsOfStall:
		...
		if button:
			...
			if button:
				if animalsOfStall[i]["node"].textures[3]:
					button.change_icon(animalsOfStall[i]["node"].textures[3])
				if animalsOfStall[i]["name"]:
					button.change_name(animalsOfStall[i]["name"])
