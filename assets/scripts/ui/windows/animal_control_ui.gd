...
@onready
var animalIcon: TextureRect = $Background/MarginContainer/HBoxContainer/MarginContainer/AnimalInfoContainer/AnimalIcon/TextureRect
@onready
var animalName: Label = $Background/MarginContainer/HBoxContainer/MarginContainer/AnimalInfoContainer/AnimalName/Label
@onready
var animalDescription: RichTextLabel = $Background/MarginContainer/HBoxContainer/MarginContainer/AnimalInfoContainer/AnimalDescription/Label
@onready
var animalButtonSell: Button = $Background/MarginContainer/HBoxContainer/MarginContainer/AnimalInfoContainer/AnimalManipulation/VBoxContainer/ButtonAnimalSell
@onready
var animalButtonChangeStall: Button = $Background/MarginContainer/HBoxContainer/MarginContainer/AnimalInfoContainer/AnimalManipulation/VBoxContainer/ButtonChangeStall
@onready
var animalButtonContainer: VBoxContainer = $Background/MarginContainer/HBoxContainer/AllStalsContainer/NinePatchRect/ScrollContainer/VBoxContainer
...
func _get_all_stalls() -> void:
	...
	for i in animalManager.get_spawns():
		...
		if button.buttonSource:
			button.buttonSource.connect("pressed", Callable(self, "_change_spawn").bind(currentAnimal, i))
		if i:
			if i.sprite:
				button.change_icon(i.sprite.get_texture())  #! Fix this

		if animalManager.get_animal(currentAnimal).size() > 0:
			if animalManager.get_animal(currentAnimal).has("house"):
				if animalManager.get_animal(currentAnimal)["house"] == i.name:
					button.buttonSource.disabled = true

		match i.blueprint_id:
			4:
				button.change_name(tr("object.animal_stall.caption"))
			_:
				button.change_name("???")
...
func _change_spawn(_animal: Object, _spawn: Object) -> void:
	...
	if notice:
		notice.create_notice(animalManager.get_animal(_animal)["name"] + " " + tr("animal.notice.has_been_moved"))
	close()


func _set_info() -> void:
	if !animalManager || !animalDescription || !currentAnimal || !animalName:
		return

	if animalManager.get_animal(currentAnimal).size() == 0:
		animalName.text = currentAnimal.animal_name
		animalDescription.text = animalManager.animalsConfig[currentAnimal.type]["description"]
	else:
		animalName.text = animalManager.get_animal(currentAnimal)["name"]
