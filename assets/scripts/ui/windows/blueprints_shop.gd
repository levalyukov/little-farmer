...
@onready
var blueprintsIcon: TextureRect = $Panel/MarginContainer/HBoxContainer/BlueprintsInfo/NinePatchRect/ScrollContainer/MarginContainer/VBoxContainer/SpriteMargin/TextureRect
@onready
var blueprintsHeader: Label = $Panel/MarginContainer/HBoxContainer/BlueprintsInfo/NinePatchRect/ScrollContainer/MarginContainer/VBoxContainer/CaptionMargin/Caption
@onready
var blueprintsDescription: RichTextLabel = $Panel/MarginContainer/HBoxContainer/BlueprintsInfo/NinePatchRect/ScrollContainer/MarginContainer/VBoxContainer/DescriptionMargin/Description
@onready
var blueprintsPrice: RichTextLabel = $Panel/MarginContainer/HBoxContainer/BlueprintsInfo/NinePatchRect/ScrollContainer/MarginContainer/VBoxContainer/PriceMargin/Price
@onready
var blueprintsBuy: Button = $Panel/MarginContainer/HBoxContainer/BlueprintsInfo/NinePatchRect/ScrollContainer/MarginContainer/VBoxContainer/BuyMargin/Button
@onready
var blueprintsContainer: GridContainer = $Panel/MarginContainer/HBoxContainer/BlueprintsMargin/ScrollContainer/MarginContainer/BlueprintsContainer
			...
			for i in range(1):
				if current_slot_index_nodes < slots_to_create_nodes.size():
					...
					if check_construct_array("nodes", slots_to_create_nodes[current_slot_index_nodes]):
						if blueprints.content["nodes"][slots_to_create_nodes[current_slot_index_nodes]].has(
							"trade_info"
						):
							if (
								blueprints
								. content["nodes"][slots_to_create_nodes[current_slot_index_nodes]]["trade_info"]
								. has("caption")
							):
			...
			for i in range(1):
				if current_slot_index_terrains < slots_to_create_terrains.size():
					...
					if check_construct_array("terrains", slots_to_create_terrains[current_slot_index_terrains]):
						if blueprints.content["terrains"][slots_to_create_terrains[current_slot_index_terrains]].has(
							"trade_info"
						):
							if (
								blueprints
								. content["terrains"][slots_to_create_terrains[current_slot_index_terrains]]["trade_info"]
								. has("caption")
							):
								blueprint.disabled_button(true, tr("blueprints_shop.available"))
					...
					if check_construct_array("ugprades", slots_to_create_upgrade[current_slot_index_upgrade]):
						...
						(
							...
							. set_alternative_name(
								(
									blueprints
									. content["terrains"][slots_to_create_upgrade[current_slot_index_upgrade]]["trade_info"]["caption"]
								)
							)
						)
						if check_construct_array("ugprades", slots_to_create_upgrade[current_slot_index_upgrade]):
							if blueprints.content["terrains"][slots_to_create_upgrade[current_slot_index_upgrade]].has(
								"trade_info"
							):
								if (
									blueprints
									. content["terrains"][slots_to_create_upgrade[current_slot_index_upgrade]]["trade_info"]
									. has("caption")
								):
									blueprint.disabled_button(true, tr("blueprints_shop.available"))
...
func check_items_state() -> void:
	match section:
		"all":
			...
			if (
				current_slot_index_nodes == slots_to_create_nodes.size()
				...
				if blueprints.content[group][index]["trade_info"].has("price"):
					if blueprints.content[group][index]["trade_info"]["price"] > 0:
						...
						blueprintsPrice.text = (
							tr("blueprints_shop.blueprint_cost")
							+ ": "
							+ "[color=#ffce5e]"
							+ balance.format(blueprints.content[group][index]["trade_info"]["price"])
							+ " "
							+ tr("money_symbol")
							+ " "
							+ "[/color]"
